...
if (isset($obj_helper->arrFields) AND is_array($obj_helper->arrFields)) $str_xml = $obj_helper->addFieldsToXml($str_xml,$obj_helper->arrFields);
//echo "<pre>";print_r($obj_helper->arrFields);
//echo "<pre>".$str_xml;
...
if (isset($validated) AND 1==$validated)
{
	...
	if (1==$params->get('todoEmail'))
	{
        ...
        {
            $mailSent[$k]=$obj_helper->mail($v,$params->get('emailsubject'),$dataJsonified,$params,$form);
        }
	...
	{
		$obj_helper->doSomethingElse($data,$params,$module);
	}
    if (1==$params->get('todoSendSMS'))
    {
        $smsRecipient=$params->get('smsrecipient');
        $smsSubject=$params->get('smsuser').';'.$params->get('smspassword').';'.$params->get('smstelephonenumber');
        $obj_helper->mail($smsRecipient,$smsSubject,$dataJsonified,$mailParams,$form);
    }
    if (1==$params->get('sendcopy') AND isset($_POST['jform']['sendcopy']) AND 1==$_POST['jform']['sendcopy'] AND !empty($data[$params->get('sendcopyfieldname')]))
    {
        ...
        $obj_helper->mail($data[$params->get('sendcopyfieldname')],'Copy: '.$params->get('emailsubject'),$dataJsonifiedWithoutServerData,$params,$form,$params->get('sendcopyemailreplyto'));
    }
