...
//[Accordion]
if(!function_exists('accordion_sc_vina')) {
	$accordionArray = array();
	function accordion_sc_vina( $atts, $content="" ){
		global $accordionArray;
		...
		foreach ($accordionArray as $key=>$accordion) {
			...
			$html .= '<div class="accordion-heading">';
			$html .= '<a class="accordion-toggle" data-toggle="collapse" data-parent="#' . $params['id'] . '" href="#'.Helix::slug($accordion['title']).'">';
			$html .= '<span class="number">'.$accordion['number'].'</span> ';
			$html .= $accordion['title'];
			$html .= '<i class="pull-right"></i>';
			$html .= '</a>';
			$html .= '</div>';//end Accordion Heading
	...
	add_shortcode( 'accordion_vina', 'accordion_sc_vina' );
		
	//Accordion Items
	function accordion_vina_item_sc( $atts, $content="" ){
		global $accordionArray;
		$accordionArray[] = array('number'=>$atts['number'],'title'=>$atts['title'], 'content'=>$content);
	}

	add_shortcode( 'accordion_vina_item', 'accordion_vina_item_sc' );			
}